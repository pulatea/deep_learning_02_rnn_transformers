...
class CaptionGenerator(BaseCaptionGenerator):
    def __init__(self, vocabulary_size, embedding_dim, hidden_dim, num_layers):
        ...
        self.q = nn.Linear(2 * self.hidden_dim, self.hidden_dim)
    ...
    def forward(self, encoded_image, caption_indices, hidden_state=None):
        ...
        if encoded_image is not None:
            ...
            q = self.q(
                torch.cat([output, rearrange(hidden_state[0], "n b h -> b 1 (n h)").expand(-1, output.size(1), -1)],
                          dim=2))
            k = self.k(encoded_image)
            v = self.v(encoded_image)
