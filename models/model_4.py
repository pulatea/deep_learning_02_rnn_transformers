...
class CaptionGenerator(BaseCaptionGenerator):
    def __init__(self, vocabulary_size, embedding_dim, hidden_dim, num_layers):
        ...
        self.attention = nn.Linear(self.hidden_dim + embedding_dim, 1)
    ...
    def forward(self, encoded_image, caption_indices, hidden_state=None):
        ...
        if encoded_image is not None:
            ...
            print("inside encoded image is not none ")
            unsqueezed_encoded_image = encoded_image.unsqueeze(1)
            unsqueezed_encoded_image = unsqueezed_encoded_image.expand(-1, output.size(1), -1)
            attention_weights = F.softmax(self.attention(torch.cat((output, unsqueezed_encoded_image), dim=2)), dim=1)
            context_vector = torch.sum(attention_weights * encoded_image.unsqueeze(1), dim=1)
