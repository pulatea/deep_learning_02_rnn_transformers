...
class CaptionGenerator(BaseCaptionGenerator):
    ...
    def forward(self, encoded_image, caption_indices, hidden_state=None):
        ...
        hidden_rearranged = rearrange(hidden, "n b h -> b 1 (n h)").expand(-1, embeddings.size(1), -1)
        concat_tensor = torch.cat([embeddings, hidden_rearranged], dim=2)
        q = self.q(concat_tensor)
        k = self.k(embeddings)
        v = self.v(embeddings)
        ...
        output = torch.cat((embeddings, context), 2)
        ...
        # Get the prediction probabilities over target language tokens
